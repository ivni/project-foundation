...
Use an independent Qwen reviewer after implementation, then let the primary agent validate and fix
the defects it reports. A clean review and passing tests are separate claims.

## Enforce the invocation and authority boundary

- Start only when the user explicitly invokes or names `run-qwen-review-loop`, or when
...
Every host must launch the actual Qwen Code CLI through `scripts/run-qwen-review.ts`; a host-native
subagent configured with a Qwen model alias is not a substitute, because it remains the host runtime
rather than an independent Qwen Code reviewer.

Require `qwen3.8-max` with `xhigh` reasoning. The wrapper pins the model as a CLI argument and pins
the reasoning effort through a wrapper-owned system settings file, which overrides user and workspace
settings; `xhigh` is also the model's documented default. If the exact profile, Qwen Code CLI,
authentication, Bun runtime, or the plan approval mode is unavailable, report the unavailable
capability and ask before using any fallback. Capability negotiation does not consume a review pass.

Authentication belongs to the Qwen Code CLI itself: the user logs in once with the CLI's own flow, or
configures a provider it supports. The wrapper never handles or requires credentials of its own, and
it keeps the user's configured authentication environment intact while stripping model, settings-path,
and sandbox overrides.

The wrapper's read-only boundary is the Qwen Code `plan` approval mode, which the Qwen Code runtime
enforces on its tool surface. That is agent-level enforcement, not an OS sandbox or filesystem
isolation: report it as such, never as whole-process isolation. The prohibition on running tests and
validation commands is an explicit reviewer contract on top of it. The wrapper also empties the MCP
server list in its pinned settings so a workspace `.qwen/settings.json` cannot hand the reviewer a
mutation tool.
...
Write this packet to an operating-system temporary file outside the repository and pass it to the
wrapper with `--context-file`. The wrapper prepends the canonical
[reviewer contract](references/reviewer-contract.md) and the output schema. Keep the combined prompt
below the wrapper's 8 MiB limit; narrow irrelevant context or stop with the reported capability
blocker. Remove the temporary context file in a `finally` step after every wrapper call, including
authentication, timeout, invalid-output, and cancellation failures. The wrapper does not delete a
caller-owned path.
...
2. Launch a fresh Qwen reviewer using the selected adapter. Keep it read-only. It may inspect code,
   diffs, and test code statically, but it must not edit files or execute tests, linters, builds, or
   other validation commands. The plan approval mode enforces the read-only boundary on the Qwen Code
   tool surface; the no-test command boundary is an explicit reviewer contract rather than a separate
   process sandbox.
...
- the verified reviewer runtime and CLI version, the pinned model and reasoning arguments, any
  CLI-reported model data, and any approved fallback; the wrapper rejects reported model usage naming
  another model, but do not claim server-side profile attestation when model data is absent;
- that the read-only boundary was the Qwen Code plan approval mode on the runtime's tool surface, and
  that the no-test boundary was reviewer-contract-only;
