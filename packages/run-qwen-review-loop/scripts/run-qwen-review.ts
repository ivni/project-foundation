...
const RUN_STATE_DIRECTORY = "qwen-review-runs";
...
const usage = `Usage:
  bun run-qwen-review.ts --context-file <path> --pass <1-10> [options]
...
/**
 * Qwen Code has no per-invocation flag for reasoning effort, so the pin travels through a
 * wrapper-owned system settings file, which overrides every user and workspace settings file. The
 * same file re-pins the approval mode and empties the MCP server list so a workspace cannot hand the
 * reviewer a mutation tool through its own `.qwen/settings.json`.
 */
export function buildQwenSettings(): Record<string, unknown> {
  return {
    model: { reasoningEffort: DEFAULT_REASONING_EFFORT },
    tools: { approvalMode: "plan" },
    mcpServers: {},
  };
}

export function buildQwenArguments(): string[] {
  return ["--output-format", "json", "--approval-mode", "plan", "--model", DEFAULT_MODEL];
}
...
/**
 * Authentication inputs such as `OPENAI_API_KEY` and `OPENAI_BASE_URL` are deliberately kept: the
 * wrapper pins the model and effort, not the credential mechanism the user configured. Successful
 * output still needs a recognizable qwen3.8-max identifier whenever the CLI reports model usage.
 */
export function buildQwenEnvironment(
  settingsPath: string,
  ...
  sanitized.QWEN_CODE_SYSTEM_SETTINGS_PATH = settingsPath;
  return sanitized;
}
 ...
 *
 * Qwen Code has no structured-output flag, so the schema travels inside the prompt and the wrapper
 * validates the parsed response against it locally.
 */
export function buildReviewerPrompt(contract: string, context: string, schema: string): string {
  return `You are the independent Qwen reviewer for this change set.
...
object matching the supplied schema, with no Markdown fences and no surrounding text.`;
}
...
export function classifyQwenFailure(output: string): RunnerErrorKind {
  if (
    /401 Unauthorized|invalid[_ ]api[_ ]key|not logged in|authentication required|token has expired|re-authenticate|please run \/auth|unauthenticated/i.test(
      output,
    )
  ) {
  ...
  if (
    /model_not_found|model[^\n]*(?:not found|unavailable|unsupported|does not exist|not enabled)|do not have access[^\n]*model|model access denied/i.test(
      output,
    )
  ) {
...
/**
 * The reviewer is contractually forbidden Markdown, but a fenced reply is recoverable without
 * trusting anything outside the fence, so the wrapper strips exactly one wrapping fence and nothing
 * more. Any other framing stays an invalid-output failure rather than a guess.
 */
export function extractReviewJson(response: string): unknown {
  const trimmed = response.trim();
  const candidate = trimmed.match(/^```(?:json)?\s*\n([\s\S]*?)\n```$/)?.[1]?.trim() ?? trimmed;
  try {
    return JSON.parse(candidate);
  } catch {
    throw new RunnerError("invalid_output", "Qwen response is not one valid JSON review result");
  }
}
...
export function reportedModels(envelope: Record<string, unknown>): string[] {
  ...
  if (isRecord(stats) && isRecord(stats.models)) {
    for (const model of Object.keys(stats.models)) models.add(model);
  }
  return [...models];
}

/**
 * Model usage in the JSON stats is corroborating evidence, not attestation: absent data is surfaced
 * as an empty `reported_models` list, while present data naming another model is a hard profile
 * failure.
 */
export function verifyReportedModels(models: string[]): void {
  const unexpected = models.filter(
    (model) => !/(?:^|[._:/-])qwen3\.8-max(?:$|[._:/-])/i.test(model),
  );
  if (unexpected.length > 0) {
    throw new RunnerError(
      "model_unavailable",
      `Qwen Code reported a runtime model other than ${DEFAULT_MODEL}: ${unexpected.join(", ")}`,
    );
  }
}

export function extractQwenDiagnostic(output: string): string {
  try {
    const envelope = JSON.parse(output);
    if (isRecord(envelope) && isRecord(envelope.error)) {
      const message = envelope.error.message;
      if (typeof message === "string" && message.trim().length > 0) return message.trim();
      return "Qwen Code returned an error envelope without a public diagnostic";
    }
  } catch {
    // Keep the raw diagnostic when Qwen Code did not emit its JSON envelope.
  }
  return output;
}

/**
 * On a nonzero exit, stdout is authoritative only when it carries the CLI's JSON error envelope.
 * Plain-text stdout is progress noise more often than a diagnostic, so the separate stderr channel
 * wins over it: preferring noisy stdout would hand `classifyQwenFailure` the wrong text and turn an
 * authentication or model-access failure into a generic one.
 */
export function selectQwenDiagnostic(stdout: string, stderr: string): string {
  const normalizedStdout = stdout.trim();
  if (normalizedStdout.length > 0) {
    try {
      const envelope = JSON.parse(normalizedStdout);
      if (isRecord(envelope) && isRecord(envelope.error)) {
        return extractQwenDiagnostic(normalizedStdout);
      }
    } catch {
...
export function parseQwenReviewEnvelope(stdoutText: string): {
  review: ReviewResult;
  ...
  try {
    ...
    throw new RunnerError("invalid_output", "Qwen output is not one valid JSON envelope");
  }
  if (!isRecord(envelope)) {
    throw new RunnerError("invalid_output", "Qwen output envelope must be an object");
  }
  if (isRecord(envelope.error)) {
    const detail = tail(extractQwenDiagnostic(stdoutText), 3000);
    throw new RunnerError(classifyQwenFailure(detail), `Qwen Code returned an error: ${detail}`);
  }
  if (typeof envelope.response !== "string" || envelope.response.trim().length === 0) {
    throw new RunnerError("invalid_output", "Qwen output envelope omitted a response");
  }
  const review = validateReviewResult(extractReviewJson(envelope.response));
  const runtimeModels = reportedModels(envelope);
  verifyReportedModels(runtimeModels);
  return { review, runtimeModels };
}

function spawnQwenProcess(
  options: ParsedArguments,
  qwenExecutable: string,
  qwenArguments: string[],
  environment: Record<string, string>,
) {
  return Bun.spawn([qwenExecutable, ...qwenArguments], {
    cwd: options.cwd,
...
type QwenProcess = ReturnType<typeof spawnQwenProcess>;
...
function resolveQwenExecutable(repositoryRoot: string): string {
  const located = Bun.which("qwen");
  if (located === null) {
    throw new RunnerError("qwen_unavailable", "could not find Qwen Code executable on PATH");
  }
  const executable = resolve(located);
  if (isInsideRepository(repositoryRoot, executable)) {
    throw new RunnerError(
      "qwen_unavailable",
      `refusing repository-local Qwen Code executable: ${executable}`,
    );
  }
  return executable;
}
 ...
 * would be the reviewed change set executing itself, so it is refused the same way Qwen Code is.
...
function verifyQwenCli(qwenExecutable: string, timeoutMs: number): string {
  let result: ReturnType<typeof Bun.spawnSync>;
  try {
    result = Bun.spawnSync([qwenExecutable, "--version"], {
      stdout: "pipe",
      stderr: "pipe",
    ...
    throw new RunnerError(
      "qwen_unavailable",
      `could not launch Qwen Code executable: ${qwenExecutable}${detail}`,
    );
  }
  if (result.exitedDueToTimeout) {
    throw new RunnerError("timeout", "Qwen Code CLI version preflight timed out");
  }
  if (result.signalCode !== undefined && result.signalCode !== null) {
    throw new RunnerError(
      "qwen_unavailable",
      `Qwen Code CLI version preflight terminated by ${result.signalCode}`,
    );
  }
  ...
  if (result.exitCode !== 0 || !/^\d+\.\d+\.\d+/.test(version)) {
    throw new RunnerError(
      "qwen_unavailable",
      "the qwen command did not report a recognizable CLI version",
    );
  }
  return version;
}

function terminateProcessTree(child: QwenProcess, force: boolean): void {
...
async function runReview(options: ParsedArguments): Promise<Record<string, unknown>> {
  ...
  const prompt = buildReviewerPrompt(contract, context, schema);
  ...
  if (elapsedBeforePreflight >= options.timeoutMs) {
    throw new RunnerError("timeout", `Qwen review exceeded ${options.timeoutMs} ms`);
  }
  const qwenExecutable = resolveQwenExecutable(options.cwd);
  const qwenVersion = verifyQwenCli(qwenExecutable, options.timeoutMs - elapsedBeforePreflight);

  const temporaryDirectory = await mkdtemp(join(tmpdir(), "qwen-review-"));
  const settingsPath = join(temporaryDirectory, "system-settings.json");
  await writeFile(settingsPath, `${JSON.stringify(buildQwenSettings(), null, 2)}\n`, {
    encoding: "utf8",
    mode: 0o600,
  });
  let child: QwenProcess | undefined;
  let timedOut = false;
    ...
    terminationDeadline = setTimeout(() => {
      ...
      rejectTerminationDeadline(
        new RunnerError(
          ...
          "Qwen process tree did not terminate after forced cancellation",
        ),
      );
  ...
  try {
    const qwenArguments = buildQwenArguments();
    const environment = buildQwenEnvironment(settingsPath);
    let subprocess: ReturnType<typeof spawnQwenProcess>;
    try {
      subprocess = spawnQwenProcess(options, qwenExecutable, qwenArguments, environment);
      child = subprocess;
    ...
    if (cancelledSignal !== undefined) {
      throw new RunnerError("cancelled", `Qwen review cancelled by ${cancelledSignal}`);
    }
    if (timedOut) {
      throw new RunnerError("timeout", `Qwen review exceeded ${options.timeoutMs} ms`);
    }
    if (exitCode !== 0) {
      const fullDiagnostic = selectQwenDiagnostic(stdoutResult.text, stderrResult.text);
      const diagnostic = tail(fullDiagnostic, 3000);
      throw new RunnerError(
        classifyQwenFailure(fullDiagnostic),
        diagnostic.length > 0
          ? `Qwen Code exited with code ${exitCode}: ${diagnostic}`
          : `Qwen Code exited with code ${exitCode}`,
      );
    }
    if (stdoutResult.truncated) {
      throw new RunnerError(
        "invalid_output",
        "Qwen Code JSON output exceeded the local safety limit",
      );
    }

    const { review, runtimeModels } = parseQwenReviewEnvelope(stdoutResult.text);
    ...
    return {
      ...
      invocation: {
        ...
        executable_path: qwenExecutable,
        cli_version: qwenVersion,
        requested_model: DEFAULT_MODEL,
        requested_reasoning_effort: DEFAULT_REASONING_EFFORT,
        profile_verification: "pinned-cli-arguments-plus-system-settings",
        reported_models: runtimeModels,
        approval_mode: "plan",
        approval_mode_enforcement: "qwen-code-runtime-tool-surface, not an OS sandbox",
...
async function main(): Promise<void> {
  try {
    ...
    const kind = error instanceof RunnerError ? error.kind : "qwen_failed";
